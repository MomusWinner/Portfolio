...
export class Game extends Scene {
	...
	create() {
		...
		let rangeStartY = 10;
		let rangeEndY = 49;
		let radius = (rangeEndX - rangeStartX) / 2

		let cX = rangeStartX + Math.floor((rangeEndX - rangeStartX) / 2)
		let cY = rangeStartY + Math.floor((rangeEndY - rangeStartY) / 2)

		for (let x = rangeStartX; x < rangeEndX; x += 1) {
			for (let y = rangeStartY; y < rangeEndY; y += 1) {
	...
	update(time: number, delta: number): void {
		...
		const nextState: boolean[][] = Array.from({ length: this.rows }, () => Array(this.cols).fill(false));
