			...
			<div className="space-y-3">
				{aliases && aliases.map((alias) => (
					...
					>
						...
						<p>Tag: <span className="text-m-green">{alias.tag}</span></p>
						<p>Alias: <span className="text-m-green">{alias.alias}</span></p>
						<DeleteButton onClick={() => { onDelete(alias) }}>Delete</DeleteButton>
					</div>
				))}
			</div>
		</div>
