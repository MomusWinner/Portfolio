...
export default function TimeIntervalList({ id, date }: Props) {
  ...
  useEffect(() => {
    async function fetchTimeIntervals() {
      ...
      if (date === undefined) {
        timeIntervals = await fetchGetAdmin<TimeInterval[]>(
          "/api/v1/time_interval/session/" + id,
          router
        );
      } else {
        timeIntervals = await fetchGetAdmin<TimeInterval[]>(
          "/api/v1/time_interval/session/" + id + "/" + formatDate(date),
          router
        );
      }
      ...
      <div className="space-y-3">
        {timeIntervals &&
          timeIntervals.map((timeInterval) => (
            ...
            >
              <div>
                <h2 className="text-lg font-semibold">{timeInterval.id}</h2>
              </div>
              <div>
                <p className="text-m-gray text-sm">
                  start: {getLocalDateTimeString(new Date(timeInterval.tStart))}
                </p>
                <p className="text-m-gray text-sm">
                  end: {getLocalDateTimeString(new Date(timeInterval.tEnd))}
                </p>
              </div>
              <p>
                {formatDuration(
                  new Date(timeInterval.tEnd).getTime() - new Date(timeInterval.tStart).getTime()
                )}{" "}
                sec
              </p>
            </div>
