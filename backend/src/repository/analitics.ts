...
export class AnaliticsRepository {
	...
	async createTimeInterval(sessionId: string, start: Date, end: Date): Promise<TimeInterval | null> {
		let interval = await createTimeInterval(this.sql, { sessionId: sessionId, tStart: start, tEnd: end });
		return interval;
	}
